...
#### 1.1. Install Chart From [WSO2 Helm Chart Repository](https://hub.helm.sh/charts/wso2)
  ...
  ```
  helm install --name <RELEASE_NAME> wso2/choreo-connect --version 1.0.0-6 --namespace <NAMESPACE>
  ```
  
  Helm version 3

  ```
  helm install <RELEASE_NAME> wso2/choreo-connect --version 1.0.0-6 --namespace <NAMESPACE> --create-namespace
  ```
...
helm install --name apim-as-cp wso2/am-single-node --version 4.0.0-1 --namespace apim \
  ...
  --set-file wso2.deployment.am.config."deployment\.toml"=https://raw.githubusercontent.com/wso2/kubernetes-microgateway/v1.0.0.6/resources/controlplane-deployment.toml
```
